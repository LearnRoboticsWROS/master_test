...
public:
    ...
    void pick()
    {
        ...
        if (success)
        {
            ...
            waypoints.push_back(pick_pose); // Posizione iniziale

            // Definisco il punto successivo (alzandomi di 10 cm in Z)
            geometry_msgs::msg::Pose post_pick_pose = pick_pose;
            post_pick_pose.position.z += 0.1;
            waypoints.push_back(post_pick_pose); // Aggiungo il waypoint finale

            // Pianificazione del percorso cartesiano
    ...
    void attachObject()
    {
        ...
        request->model2_name = "sponge"; // Nome dell'oggetto da afferrare
