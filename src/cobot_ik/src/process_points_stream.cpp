...
private:
    ...
    // Funzione per applicare il filtro Voxel Grid
    pcl::PointCloud<pcl::PointXYZ>::Ptr applyVoxelFilter(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, float leaf_size) {
        ...
        voxel_filter.setLeafSize(leaf_size, leaf_size, leaf_size); // Imposta la dimensione dei voxel
        voxel_filter.filter(*cloud_filtered); // Esegui il filtro
        return cloud_filtered;
    }

    // Funzione per applicare il filtro PassThrough
    ...
    // Funzione per pubblicare una PointCloud
    void publishPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, 
                           rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& publisher, 
                           const std::string& frame_id) {
        sensor_msgs::msg::PointCloud2 cloud_msg;
        pcl::toROSMsg(*cloud, cloud_msg); // Converte da PCL a ROS2 PointCloud2
        cloud_msg.header.stamp = this->now();
        cloud_msg.header.frame_id = frame_id; // Imposta il frame di riferimento
        publisher->publish(cloud_msg); // Pubblica la PointCloud
    }

    // Funzione per generare waypoints da una PointCloud filtrata
    pcl::PointCloud<pcl::PointXYZ>::Ptr generateWaypoints(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_filtered, float step_x) {
        ...
        // Crea un waypoint per ogni bin dell'asse X
        for (const auto& point : cloud_filtered->points) {
            int x_bin = static_cast<int>(point.x / step_x); // Mappa i punti in base a step_x
            if (waypoints_map.find(x_bin) == waypoints_map.end() || point.z < waypoints_map[x_bin].z) {
                waypoints_map[x_bin] = point; // Conserva il punto con la Z minima
            }
        }

        pcl::PointCloud<pcl::PointXYZ>::Ptr waypoints(new pcl::PointCloud<pcl::PointXYZ>);
        for (const auto& entry : waypoints_map) {
            waypoints->points.push_back(entry.second); // Aggiungi i waypoints al risultato
        }
